...
2. **Post a `status` when you start a task, and again whenever your scope grows**: a new directory, a new repo, a shared file you did not expect to touch. That expansion is when a collision becomes likely. Under Claude Code a hook keeps this in step with your todo list, so the in-progress item is already on the board; every other client posts it or has nothing. Post one yourself whenever your scope is wider than the item you are on — a todo says what you are doing, not which files you have claimed.
